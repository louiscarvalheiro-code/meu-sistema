...
def create_tables_and_defaults():
    try:
        ...
        app.logger.error('Erro a criar tabelas: %s', e)
    # default Diversos (actualizados conforme pedido)
    defaults = {
    ...
    # example compositions (only add if not present)
    mistura_exemplos = {
...
def show_routes():
    ...
    for rule in app.url_map.iter_rules():
        routes.append({'endpoint': rule.endpoint, 'methods': list(rule.methods), 'rule': str(rule)})
...
def calculo():
    ...
    if request.method == 'POST':
        try:
            ...
            app.logger.error('Erro no calculo: %s', e)
...
@app.route('/misturas/<int:id>/delete', methods=['POST'])
