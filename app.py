...
from models import db, Material, Mistura, Composicao, Equipamento, Humano, Diverso
...
def seed_data():
    ...
    for mistura_nome, comps in mistura_exemplos.items():
        ...
        for mat_nome, pct in comps:
            ...
            if mat and not Composicao.query.filter_by(mistura_id=mistura.id, material_id=mat.id).first():
                db.session.add(Composicao(mistura_id=mistura.id, material_id=mat.id, percentagem=pct))
    db.session.commit()
...
def custo_mistura_por_ton(mistura_id):
    custo = 0.0
    comps = Composicao.query.filter_by(mistura_id=mistura_id).all()
    for c in comps:
...
def misturas_composicao(mistura_id):
    ...
    if request.method == 'POST':
        ...
        existing = Composicao.query.filter_by(mistura_id=mistura_id, material_id=material_id).first()
        if existing:
            flash('Material já existe na mistura. Edite a percentagem.', 'warning')
        else:
            new_total = db.session.query(func.coalesce(func.sum(Composicao.percentagem), 0.0)).filter_by(mistura_id=mistura_id).scalar() or 0.0
            new_total += percent
            if new_total > 100.0001:
                flash(f'A soma das percentagens excede 100% (total atual: {new_total}%).', 'danger')
                return redirect(url_for('misturas_composicao', mistura_id=mistura_id))
            comp = Composicao(mistura_id=mistura_id, material_id=material_id, percentagem=percent)
            db.session.add(comp)
            db.session.commit()
            flash('Componente adicionado.', 'success')
        return redirect(url_for('misturas_composicao', mistura_id=mistura_id))
    total_pct = db.session.query(func.coalesce(func.sum(Composicao.percentagem), 0.0)).filter_by(mistura_id=mistura_id).scalar() or 0.0
...
def misturas_composicao_edit(mistura_id, comp_id):
    comp = Composicao.query.get_or_404(comp_id)
    nova = float(request.form.get('percentagem') or 0.0)
    soma_outras = db.session.query(func.coalesce(func.sum(Composicao.percentagem), 0.0)).filter_by(mistura_id=mistura_id).scalar() - comp.percentagem
